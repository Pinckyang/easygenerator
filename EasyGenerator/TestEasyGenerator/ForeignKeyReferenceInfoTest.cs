...
namespace TestEasyGenerator
{
    ...
    public class ForeignKeyReferenceInfoTest
    {
        ...
        [TestMethod()]
        public void ForeignKeyReferenceInfoConstructorTest2()
        {
            ReferenceInfo target = new ForeignKeyReferenceInfo();
            target.ReferenceColumnName = "ReferenceColumn1";
            target.ReferenceTableName = "ReferenceTable1";
            target.ColumnName = "column1";
            target.Name = "Reference1";
            ...
            column.DBControlType = DBControlType.DBEdit;
            ...
            try
            {
                XmlSerializer xmlSerializer = new XmlSerializer(column.GetType());
                StringWriter writer = new StringWriter();
                xmlSerializer.Serialize(writer, column);
                StringReader reader = new StringReader(writer.ToString());
                ColumnInfo actual = (ColumnInfo)xmlSerializer.Deserialize(reader);
                Assert.AreEqual("column1", actual.Name);
            }
            catch (Exception ex)
            {
