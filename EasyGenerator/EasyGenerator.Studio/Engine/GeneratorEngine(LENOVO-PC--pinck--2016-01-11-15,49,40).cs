...
{
    public delegate void GenerateFile(OutputFile file);
    public class GeneratorEngine
    {
        ...
        public event GenerateFile OnGenerateFile=null;
        ...
        public void LoadTemplates(string templateDir)
        {
            ...
            GenerateChildTempates(templateDir, string.Empty, string.Empty, "UTF8", false);
        }

        private void GenerateChildTempates(string templateDir,string pattern,string matchValue,string charset,bool native)
        {
            string[] childFolders = Directory.GetDirectories(templateDir, "*");
            List<string> folders = new List<string>(childFolders);
            //folders.Insert(0, templateDir);

            List<OutputFile> files = GenerateTempleteFiles(this.project, templateDir, matchValue,charset,native);
            this.outputFiles.AddRange(files.ConvertAll<OutputFile>(e => 
            { 
                string outputFolder=Regex.Replace(e.OutputFolder, pattern, matchValue, RegexOptions.IgnoreCase);
                outputFolder = Regex.Replace(outputFolder, "\\[CHARSET=.*?\\]", string.Empty, RegexOptions.IgnoreCase);
                outputFolder = Regex.Replace(outputFolder, "\\[NATIVE=.*?\\]", string.Empty, RegexOptions.IgnoreCase);
                outputFolder = Regex.Replace(outputFolder, pattern, matchValue, RegexOptions.IgnoreCase);
                e.OutputFolder = Regex.Replace(outputFolder.Replace("\\", "/"), this.templateDir.Replace("\\", "/").TrimEnd('/'), string.Empty, RegexOptions.IgnoreCase).Replace("/", "\\");
                return e; 
            }));
        ...
        private static List<OutputFile> GenerateTempleteFiles(Project pro, string directory, string matchValue, string charset, bool native)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (matchs.Count < 1)
                {
                    ...
                    outputFile.EntityName = string.Empty;
                    outputFile.Charset = charset;
                    outputFile.Native = native;
                    outputFile.OutputFolder = Regex.Replace(fileInfo.DirectoryName, "\\[PROJECTNAME\\]", pro.Name, RegexOptions.IgnoreCase);
                ...
                foreach (Match match in matchs)
                {
                    if (match.Value.ToUpper() == "[TABLES]")
                    {
                        foreach (EntityInfo entity in pro.Database.Tables.Values)
                        {
                            ...
                            outputFile.EntityName = entity.Name;
                            outputFile.Charset = charset;
                            outputFile.Native = native;
                            outputFile.OutputFolder = Regex.Replace(fileInfo.DirectoryName, "\\[PROJECTNAME\\]", pro.Name, RegexOptions.IgnoreCase);
                            outputFile.FileText = File.ReadAllBytes(file);
                    ...
                    else if (match.Value.ToUpper() == "[MODULES]")
                    {
                        foreach (Model.Module module in pro.Ui.SystemModule.Modules.Values)
                        {
                            ...
                            outputFile.EntityName = module.Name;
                            outputFile.Charset = charset;
                            outputFile.Native = native;
                            outputFile.OutputFolder = Regex.Replace(fileInfo.DirectoryName, "\\[PROJECTNAME\\]", pro.Name, RegexOptions.IgnoreCase);
                            outputFile.FileText = File.ReadAllBytes(file);
                        ...
                        if (matchValue != string.Empty)
                        {
                            ...
                            if (module != null)
                            {
                                foreach (Window window in module.Windows.Values)
                                {
                                    ...
                                    outputFile.EntityName = window.Name;
                                    outputFile.Charset = charset;
                                    outputFile.Native = native;
                                    outputFile.OutputFolder = Regex.Replace(fileInfo.DirectoryName, "\\[PROJECTNAME\\]", pro.Name, RegexOptions.IgnoreCase);
                                    outputFile.FileText = File.ReadAllBytes(file);
                    ...
                    else if (match.Value.ToUpper() == "[DIALOGS]")
                    {
                        foreach (Dialog dialog in pro.Ui.CommonModule.Dialogs.Values)
                        {
                            ...
                            outputFile.EntityName = dialog.Name;
                            outputFile.Charset = charset;
                            outputFile.Native = native;
                            outputFile.OutputFolder = Regex.Replace(fileInfo.DirectoryName, "\\[PROJECTNAME\\]", pro.Name, RegexOptions.IgnoreCase);
                            outputFile.FileText = File.ReadAllBytes(file);
        ...
        public void GenorateFiles(string outputDir)
        {
            ...
            foreach (OutputFile file in outputFiles)
            {
                if (!file.Native)
                {
                    ...
                    File.WriteAllText(string.Format("{0}\\{1}", outputDir, file.FileName), result);
                }
                else
                {
                    File.WriteAllBytes(string.Format("{0}\\{1}", outputDir, file.FileName), file.FileText);
                }

                if (this.OnGenerateFile != null)
                {
                    OnGenerateFile(file);
                }
            }
